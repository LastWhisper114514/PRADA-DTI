...
from backbones.PSICHIC_.pna import PNAConv
import torch
...
class Protein_PNAConv(nn.Module):
    def __init__(self, prot_deg, hidden_channels, edge_channels,
        ...
        # self.lora_A = nn.Linear(hidden_channels, r, bias=False)
        # self.lora_B = nn.Linear(r, hidden_channels, bias=False)

        # 默认 mask=1（启用 LoRA）
        self.register_buffer("lora_mask", torch.ones(1))

        # nn.init.normal_(self.lora_A.weight, std=1e-3)
    ...
    # def cache_old_lora(self):
    #     # 在 end_task 时调用，保存旧任务权重
    #     self.old_lora_A = self.lora_A.weight.detach().clone()
    #     self.old_lora_B = self.lora_B.weight.detach().clone()
    ...
    def forward(self, x, prot_edge_index, prot_edge_attr):
        ...
        # out = self.conv(x, prot_edge_index, prot_edge_attr)
        # out = self.norm(out)
        # out = out + self.scaling * self.lora_B(self.lora_A(out)) * self.lora_mask.item()
        #
        #
        # x = x_in + F.relu(out)
...
class DrugProteinConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # LoRA 低秩参数
        self.lora_A_key = nn.Linear(residue_channels, r, bias=False)
        self.lora_B_key = nn.Linear(r, heads * self.atom_out_channels, bias=False)

        self.lora_A_query = nn.Linear(atom_channels, r, bias=False)
        self.lora_B_query = nn.Linear(r, heads * self.atom_out_channels, bias=False)

        self.lora_A_value = nn.Linear(residue_channels, r, bias=False)
        self.lora_B_value = nn.Linear(r, heads * self.atom_out_channels, bias=False)

        self.lora_A_atom_value = nn.Linear(atom_channels, r, bias=False)
        self.lora_B_atom_value = nn.Linear(r, heads * self.residue_out_channels, bias=False)

        # 初始化
        for lora_layer in [self.lora_A_key, self.lora_B_key,
                           self.lora_A_query, self.lora_B_query,
                           self.lora_A_value, self.lora_B_value,
                           self.lora_A_atom_value, self.lora_B_atom_value]:
            nn.init.normal_(lora_layer.weight, std=1e-2)
    ...
    def reset_lora_weights(self):
        """初始化 LoRA 参数"""
        for layer in [self.lora_A_key, self.lora_B_key,
                      self.lora_A_query, self.lora_B_query,
                      self.lora_A_value, self.lora_B_value,
                      self.lora_A_atom_value, self.lora_B_atom_value]:
    ...
    def get_lora_weights(self):
        """返回当前层的所有 LoRA 权重"""
        return {
            "A_key": self.lora_A_key.weight.detach().clone(),
            "B_key": self.lora_B_key.weight.detach().clone(),
            "A_query": self.lora_A_query.weight.detach().clone(),
            "B_query": self.lora_B_query.weight.detach().clone(),
            "A_value": self.lora_A_value.weight.detach().clone(),
            "B_value": self.lora_B_value.weight.detach().clone(),
            "A_atom_value": self.lora_A_atom_value.weight.detach().clone(),
            "B_atom_value": self.lora_B_atom_value.weight.detach().clone(),
        }
    def load_lora_weights(self, weights):
        """加载指定任务的 LoRA 权重"""
        self.lora_A_key.weight.data.copy_(weights["A_key"])
        self.lora_B_key.weight.data.copy_(weights["B_key"])
        self.lora_A_query.weight.data.copy_(weights["A_query"])
        self.lora_B_query.weight.data.copy_(weights["B_query"])
        self.lora_A_value.weight.data.copy_(weights["A_value"])
        self.lora_B_value.weight.data.copy_(weights["B_value"])
        self.lora_A_atom_value.weight.data.copy_(weights["A_atom_value"])
        self.lora_B_atom_value.weight.data.copy_(weights["B_atom_value"])
    ...
    def forward(self, drug_x, clique_x, clique_batch, residue_x, edge_index: Adj):
        ...
        # 原始线性层输出
        query = self.lin_query(drug_x)
        key = self.lin_key(residue_hx)
        value = self.lin_value(residue_hx)

        # LoRA 增量
        query += self.scaling * self.lora_B_query(self.lora_A_query(drug_x))
        key += self.scaling * self.lora_B_key(self.lora_A_key(residue_hx))
        value += self.scaling * self.lora_B_value(self.lora_A_value(residue_hx))

        # reshape 成多头
        query = query.view(-1, H, aC)
        key = key.view(-1, H, aC)
        value = value.view(-1, H, aC)
        ...
        try:
            assert clique_x.shape[0] == clique_batch.shape[0], \
                f"[Shape mismatch] clique_x.shape = {clique_x.shape}, " \
                f"clique_batch.shape = {clique_batch.shape}, " \
                f"drug_out.shape = {drug_out.shape}"

            assert drug_out[clique_batch].shape[0] == clique_x.shape[0], \
                f"[Mismatch] drug_out[clique_batch].shape = {drug_out[clique_batch].shape}, " \
                f"clique_x.shape = {clique_x.shape}"

            # 如果你的数据对象里包含 sample key，例如 mol_key，可以进一步打印
            if hasattr(clique_x, 'mol_key'):
                print(f"[Error Sample] mol_key: {clique_x.mol_key}")

        except AssertionError as e:
            print("Detected shape mismatch before torch.cat:")
            print("clique_x       :", clique_x.shape)
            print("drug_out       :", drug_out.shape)
            print("clique_batch   :", clique_batch.shape)
            print("drug_out[...]  :", drug_out[clique_batch].shape)
            raise e
        ...
        residue_value = self.lin_atom_value(drug_hx)
        residue_value += self.scaling * self.lora_B_atom_value(self.lora_A_atom_value(drug_hx))
        residue_value = residue_value.view(-1, H, rC)[edge_index[1]]
