...
from backbones.PSICHIC_.layers import MLP, AtomEncoder, Drug_PNAConv, Protein_PNAConv, DrugProteinConv, PosLinear, \
    GCNCluster, SAGECluster, SGCluster, APPNPCluster, dropout_edge
from copy import deepcopy
## for drug pooling
from backbones.PSICHIC_.drug_pool import MotifPool
## for cluster
from torch_geometric.utils import dense_to_sparse, to_dense_adj, to_dense_batch, dropout_adj, degree, subgraph, softmax, \
    add_remaining_self_loops
from backbones.PSICHIC_.protein_pool import dense_mincut_pool, dense_dmon_pool, simplify_pool
## for cluster
...
class net(torch.nn.Module):
    def __init__(self, mol_deg, prot_deg,
                 # MOLECULE
                 mol_in_channels, prot_in_channels, prot_evo_channels,
                 hidden_channels,
                 ...
                 drop_atom,
                 drop_residue,
                 dropout_cluster_edge,
                 gaussian_noise,
                 # objective
                 regression_head,
                 classification_head,
                 multiclassification_head,  ## or any number
                 use_prompt=False,
                 device='cuda:0'):
        ...
        for idx in range(total_layer):
            ...
            # )
            # self.prot_convs.append(LoRAWrapper(base_layer, out_dim=hidden_channels, r=4, alpha=1.0))
        ...
        if use_prompt:
            self.prompt_proj_prot = nn.Linear(200, hidden_channels)  # 200:prompt_dim

        if self.regression_head:
            self.reg_out = MLP([hidden_channels * 2, hidden_channels, 1])
        if self.classification_head:
            self.cls_out = MLP([hidden_channels * 2, hidden_channels, 2])
    ...
    def get_query(self,
                  mol_pool_feat: torch.Tensor,
                  prot_pool_feat: torch.Tensor):
        """
        用于 prompt 匹配的 query 向量（即 interaction_fingerprint）。
        完全绕过 forward，不触发预测 head。
        """
        mol_out = self.mol_out(mol_pool_feat)
        prot_out = self.prot_out(prot_pool_feat)
        interaction_fingerprint = torch.cat([mol_out, prot_out], dim=-1)  # [B, D]
    ...
    def forward(self,
                ...
                mol_x_batch=None, prot_node_aa_batch=None, clique_x_batch=None,
                ## only if you're interested in clustering algorithm
                save_cluster=False,
                prompt: torch.Tensor = None, returnt=None):
        # Init variables
        reg_pred = None
        cls_pred = None
        mcls_pred = None
        ...
        # PROTEIN Featurize
        prot_evo_out = self.prot_evo(residue_evo_x)
        prot_aa_out = self.prot_aa(residue_x)

        residue_x = prot_aa_out + prot_evo_out
        ratio = prot_evo_out.norm().item() / (prot_aa_out.norm().item() + 1e-6)
        if ratio < 0.2:  # 当 prot_evo 占比 <20% 时打印
            print(f"[Prot Debug] prot_evo/prot_aa ratio = {ratio:.4f}")
        ...
        for idx in range(self.total_layer):
            ...
            ## Pool Drug
            drug_x, clique_x, clique_score = self.mol_pools[idx](atom_x, clique_x, atom2clique_index, clique_x_batch,
                                                                 clique_edge_index)
            ...
            residue_hx, residue_mask = to_dense_batch(residue_x, prot_node_aa_batch)

            if save_cluster:
                layer_s[idx] = s

                # cluster features
            s, _ = to_dense_batch(s, prot_node_aa_batch)
            residue_adj = to_dense_adj(residue_edge_index, prot_node_aa_batch)
            ...
            if self.drop_residue != 0 and self.training:
                ...
                residue_drop_mask, _ = to_dense_batch(residue_drop_mask.reshape(-1, 1),
                                                      prot_node_aa_batch)  # drop residue for regularization
            ...
            if returnt == "query":
                query = cluster_x.view(batch_size, self.num_cluster[idx], -1).mean(dim=1)
                return query  # ✅

            # if prompt is not None:
            #     # prompt shape: [B, prompt_len, D] or [B, D] (after mean)
            #     # cluster_x: [B * num_cluster, D]
            #     # 假设每个样本 prompt 已经 mean 掉了：[B, D]
            #     expanded_prompt = prompt.repeat_interleave(self.num_cluster[idx], dim=0)  # [B * num_cluster, D]
            #     prompt_feat = self.prompt_proj_prot(expanded_prompt)  # [80, 5, 200]
            #     prompt_feat = prompt_feat.mean(dim=1)
            #     cluster_x = cluster_x + prompt_feat

            ## model interative relationship
            clique_x, cluster_x, inter_attn = self.inter_convs[idx](drug_x, clique_x, clique_x_batch, cluster_x,
                                                                    p2m_edge_index)
            ...
            residue_hx, _ = to_dense_batch(cluster_x.to(cluster_residue_batch.device), cluster_residue_batch)
            inter_attn, _ = to_dense_batch(inter_attn.to(cluster_residue_batch.device), cluster_residue_batch)
            residue_x = residue_x + F.relu(self.residue_lins[idx]((s @ residue_hx)[residue_mask]))  # cluster -> residue
            residue_x = residue_x.to(cluster_residue_batch.device) + self.c2r_mlps[idx](residue_x)
            ...
            ## Graph Normalization
            atom_x = self.mol_gn2[idx](atom_x, mol_x_batch)
            residue_x = self.prot_gn2[idx](residue_x, prot_node_aa_batch)
        ...
        atom_score = softmax(atom_score, mol_x_batch)
        mol_pool_feat = global_add_pool(atom_x * atom_score, mol_x_batch)

        residue_scores = torch.cat(residue_scores, dim=-1)
        residue_score = softmax(self.residue_attn_lin(residue_scores), prot_node_aa_batch)
        prot_pool_feat = global_add_pool(residue_x * residue_score, prot_node_aa_batch)
        ...
        #     if prompt.dim() == 3:
        #         # prompt shape: [B, prompt_len, D]
        #         prompt_feat = prompt.mean(dim=1)  # 平均池化为 [B, D]
        #     elif prompt.dim() == 2:
        #         prompt_feat = prompt
        #     else:
        #         raise ValueError("Prompt must be 2D or 3D tensor")
        #
        #     # 方法一：拼接（推荐，保持信息独立性）
        #     mol_prot_feat = torch.cat([mol_prot_feat, prompt_feat], dim=-1)

        # 方法二（可选）：加和（必须 dim 对齐）
        # mol_prot_feat = mol_prot_feat + prompt_feat
        ...
        attention_dict = {
            ...
            'drug_atom_index': mol_x_batch,
            'drug_clique_index': clique_x_batch,
            'protein_residue_index': prot_node_aa_batch,
...
def _rbf(D, D_min=0., D_max=1., D_count=16, device='cpu'):
    ...
    shape [...dims, D_count].
    '''
    # TODO:原始代码：D = torch.where(D < D_max, D, torch.tensor(D_max).float().to(device) )
    ...
    # TODO:RBF = torch.exp(-((D_expand - D_mu) / D_sigma) ** 2)

    device = D_expand.device
    D_mu = D_mu.to(device)
    D_sigma = torch.tensor(D_sigma, device=device)
